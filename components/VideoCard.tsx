...
const VideoCard: React.FC<VideoCardProps> = ({ video }) => {
  return (
    <a
      ...
      className="group bg-gray-800 rounded-xl overflow-hidden shadow-lg border border-gray-700 hover:border-wow-gold transition-all duration-300 transform hover:-translate-y-1 hover:shadow-[0_14px_28px_-12px_rgba(248,183,0,0.35)] block"
    >
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {video.tags?.slice(0, 3).map((tag, idx) => (
            <span
              key={idx}
              className="text-[10px] uppercase tracking-wider bg-gray-700 text-gray-300 px-2 py-1 rounded border border-gray-600"
            >
              {tag}
            </span>
          ))}
